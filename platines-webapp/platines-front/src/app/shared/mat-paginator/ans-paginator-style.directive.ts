...
@Directive({
  selector: '[ansPaginatorStyle]'
})
export class AnsPaginatorStyleDirective {
  private _pageGapTxt = "...";
  ...
  private buildPageNumbers() {
    ...
    const matMdcPaginatorRangeLabel = this.vr.element.nativeElement.querySelector(
      "div.mat-mdc-paginator-range-label"
    );

    const matMdcPaginatorContainer = this.vr.element.nativeElement.querySelector(
        ...
        for (let i = 0; i < nodeArray.length; i++) {
          if (nodeArray[i].nodeName === "BUTTON") {
            if (nodeArray[i].innerHTML.length > 100 && nodeArray[i].disabled) {
              this.ren.addClass(nodeArray[i], "page-prev");
              this.ren.addClass(nodeArray[i], "page-item");
              this.ren.addClass(nodeArray[i], "page-link");
              this.ren.addClass(nodeArray[i], "disabled");
              this.ren.removeClass(nodeArray[i], "mat-mdc-icon-button");
              this.ren.removeClass(nodeArray[i], "mat-mdc-button-base");
              this.ren.removeClass(nodeArray[i], "mdc-icon-button");
            } else if (
              nodeArray[i].innerHTML.length > 100 &&
              !nodeArray[i].disabled
            ) {
              this.ren.removeClass(nodeArray[i], "disabled");
    ...
    //Move mat-mdc-paginator-range-label one level up
    this.ren.insertBefore(
      matMdcPaginatorContainer,
      matMdcPaginatorRangeLabel,
      actionContainer
    );
  }
  ...
  private createButton(i: any, pageIndex: number): any {
    ...
    this.ren.addClass(linkBtn, "page-item");
    this.ren.addClass(linkBtn, "page-link");
    ...
    switch (i) {
      case pageIndex:
        ...
        this.ren.addClass(linkBtn, "page-number");
        break;
