...
mvn -o -DskipTests org.codehaus.mojo:build-helper-maven-plugin:timestamp-property@timestamp-property license:check >$log
status=$?
...
if [ $status -ne 0 ]; then
	...
	if [ $(grep Missing $log | wc -l) -eq 0 ]; then
		...
		echo mvn org.codehaus.mojo:build-helper-maven-plugin:timestamp-property@timestamp-property license:format
		rm $log
	fi
	exit 1
fi
