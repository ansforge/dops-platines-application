job "platines-dev" {
	datacenters = ["henix_qualif1"]
	type = "service"
	update {
		stagger = "30s"
		max_parallel = 1
	}

	group "db-servers" {
		count = "1"
		constraint {
			attribute = "${attr.unique.hostname}"
			value = "vmc6c20e44.qual.henix.asip.hst.fluxus.net"
		}
		restart {
			attempts = 3
			delay = "60s"
			interval = "1h"
			mode = "fail"
		}
		task "mariadb" {
			driver = "docker"
			config {
				image = "rhscl/mariadb-101-rhel7:latest"
					volumes = ["/opt/nomad/data/platines-dev:/var/lib/mysql/data"]
				port_map {
					db = 3306
				}
			}
			env {
				"MYSQL_USER"     = "user1_ror"
				"MYSQL_PASSWORD" = "password"
				"MYSQL_DATABASE" = "db_ror"
			}
			resources {
				cpu = 500
				memory = 1024
				network {
					port "db" {}
					mbits = 20
				}
			}
			service {
				name = "${NOMAD_JOB_NAME}-mariadb-server"
				tags = ["asipsante.${NOMAD_JOB_NAME}"]
				port = "db"
				check{
					type = "tcp"
					port = "db"
					name = "check_mysql"
					interval = "60s"
					timeout = "10s"
					check_restart {
						limit = 3
						grace = "120s"
						ignore_warnings = true
					}
				}
			}
		}
	}

	group "apps-servers" {
		count = "1"
		ephemeral_disk {
			size = 200
		}
		task "tomcat" {
			artifact {
				source = "http://repo.proxy-dev-forge.asip.hst.fluxus.net/artifactory/repo-snapshots/fr/asipsante/platines/platines-back-web/1.2.1-SNAPSHOT/platines-back-web-1.2.1-SNAPSHOT.war"
				mode = "file"
				destination = "tmp/platines-api.war"
			}
			artifact {
				source = "http://repo.proxy-dev-forge.asip.hst.fluxus.net/artifactory/repo-snapshots/fr/asipsante/platines/platines/1.2.1-SNAPSHOT/platines-1.2.1-SNAPSHOT.war"
				mode = "file"
				destination = "tmp/platines.war"
			}
			artifact {
				source = "http://repo.proxy-dev-forge.asip.hst.fluxus.net/artifactory/repo-snapshots/fr/asipsante/platines/tomcat/1.0.0/extensions.tgz"
				destination = "local"
			}
			
			driver = "docker"
			config {
				image = "jboss-webserver-3/webserver31-tomcat8-openshift:latest"
				volumes = ["tmp:/deployments","local:/opt/webserver/extensions"]
				port_map {
					http = 8080
				}
			}
			env {
				TOMCAT_ADDR="${NOMAD_ADDR_http}"
				VARIABLE_DE_REDEPLOIEMENT="c"
			}

			
				
			template {
				data = <<EOH
					GENERIC_PASSWORD_SSL="QmlRMMjPBmWwHTrtRHDadBIeDssW0niR/SqshcuP5YspXljs"
					PROTOCOL_API="http"
					SECURE_PROTOCOL_API="https"
					WEBAPPS_FOLDER="/deployments"
					WORKSPACE_DIR="mocks"
					PORT_API=":8080"
					CONTEXT_API="platines-api"
					API_FOLDER="/deployments/platines-api"
					API_DOMAIN="platines.qualif1.henix.asipsante.fr"
					MOCK_SERVICE_DOMAIN="mock.platines.henix.asipsante.fr"
					SERVER_API="localhost"
					PROXY_HOST="10.0.49.163"
					PROXY_PORT="3128"
					SMTP_HOST="smtpout.francenet.fr"
					SMTP_PORT="25"
					SMTP_PROXY_HOST=""
					SMTP_PROXY_PORT=""
					JAVA_INITIAL_MEM_RATIO="100"
					JAVA_MAX_MEM_RATIO="80"
					DB_SERVICE_PREFIX_MAPPING="MYSQL"
					JOB_MOCK_SERVICE_TMPL="/deployments/mock.nomad.json"
					{{range service "http.nomad"}}
					NOMAD_API_PORT="{{.Port}}"{{end}}
					{{range service ( print (env "NOMAD_JOB_NAME") "-mariadb-server") }}
					MYSQL_SERVICE_HOST="{{.Address}}"
					MYSQL_SERVICE_PORT="{{.Port}}"{{end}}
					MYSQL_JNDI="jdbc/rorJNDI"
					MYSQL_USERNAME="user1_ror"
					MYSQL_PASSWORD="password"
					MYSQL_DATABASE="db_ror"
					JOB_TMPL_REPOSITORY="http://repo.proxy-dev-forge.asip.hst.fluxus.net/artifactory/nomad-tmpl/fr/asipsante/platines/testing"
					CATALINA_OPTS_APPEND="-Duser.timezone=Europe/Paris"
				EOH
				destination = "tmp/env"
				change_mode = "restart"
				env         = true
			}

			resources {
				cpu = 500
				memory = 2048
				network {
					port "http" {}
					mbits = 10
				}
			}
			service {
				name = "${NOMAD_JOB_NAME}-tomcat-server"
				tags = ["asipsante.${NOMAD_JOB_NAME}"]
				port = "http"
				check {
					type = "http"
					port = "http"
					path = "/platines"
					name = "check_tomcat"
					interval = "40s"
					timeout = "10s"
				}
			}
		}
	}


	group "front-servers" {
		count = "1"
		constraint {
			attribute = "${attr.unique.hostname}"
			value = "vmc6c20e41.qual.henix.asip.hst.fluxus.net"
		}

		task "haproxy" {
			driver = "docker"
			config {
				image = "haproxytech/haproxy:latest"
				volumes = ["local:/etc/haproxy"]
				port_map {
					ha_https = 8443
					ha_http = 8080
				}
			}
			env {
				"PUBLIC_HOSTNAME" = "platines.qualif1.henix.asipsante.fr"
			}
			template {
data = <<EOH
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN RSA PRIVATE KEY-----

Q5exaWDEPT9YEPr959mwqgOU4NQwkUj1yBhUnR+ay2uwsH8X8tSC
-----END RSA PRIVATE KEY-----

EOH
				destination = "local/platines.pem"
			}

			artifact {
				source = "http://repo.proxy-dev-forge.asip.hst.fluxus.net/artifactory/nomad-tmpl/fr/asipsante/platines/testing/haproxy.tmpl"
				destination = "/tmp"
			}

			template {
				source = "tmp/haproxy_test.tmpl"
				destination = "local/haproxy.cfg"
				change_mode = "signal"
				change_signal = "SIGUSR1"
			}

			resources {
				cpu = 300
				memory = 512
				network {
					port "ha_https" {static = "8443"}
					port "ha_http"  {static = "8080"}
					mbits = 10
				}
			}
			service {
				name = "${NOMAD_JOB_NAME}-haproxy"
				tags = ["asipsante.${NOMAD_JOB_NAME}"]
				port = "ha_https"
				check {
					type = "tcp"
					port = "ha_https"
					name = "check_haproxy"
					interval = "40s"
					timeout = "1s"
				}
			}
		}
	}
	
	group "front-servers-mock" {
		count = "1"
		constraint {
			attribute = "${attr.unique.hostname}"
			value = "vmc6c20e41.qual.henix.asip.hst.fluxus.net"
		}

		task "haproxy-mock" {
			driver = "docker"
			config {
				image = "haproxytech/haproxy:latest"
				volumes = ["local:/etc/haproxy"]
				port_map {
					ha_mock = 8444
				}
			}

			artifact {
				source = "http://repo.proxy-dev-forge.asip.hst.fluxus.net/artifactory/nomad-tmpl/fr/asipsante/platines/testing/haproxy_mock.tmpl"
				destination = "/tmp"
			}

			template {
				source = "tmp/haproxy_mock.tmpl"
				destination = "local/haproxy.cfg"
				change_mode = "restart"
			}

			resources {
				cpu = 300
				memory = 512
				network {
					port "ha_mock"  {static = "8444"}
					mbits = 10
				}
			}
			service {
				name = "${NOMAD_JOB_NAME}-haproxy-mock"
				tags = ["asipsante.${NOMAD_JOB_NAME}-mock"]
				port = "ha_mock"
				check {
					type = "tcp"
					port = "ha_mock"
					name = "check_haproxy_mock"
					interval = "40s"
					timeout = "1s"
				}
			}
		}
	}
}
